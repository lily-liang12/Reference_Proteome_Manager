...
class GUI:
    ...
    def __init__(self, url, ref_prot_path, text, headers, banned_list):
        ...
        self.text = text                    # HTML text of all animals in table format
        self.raw_table = []
    ...
    def create_gui(self):
        ...
        for col in self.headers:
            if col in ["TAX ID"]:
                ...
                self.tree_left.column(col, minwidth=25, width=75, stretch=NO, anchor=E)
            else:
                self.tree_left.heading(col, text=col.title(), anchor=W,
                                       command=lambda col_=col: self.sort_text_column(self.tree_left, col_))
                self.tree_left.column(col, minwidth=25, width=75, stretch=NO)
        ...
        for col in self.headers:
            if col in ["TAX ID"]:
                ...
                self.tree_right.column(col, minwidth=25, width=75, stretch=NO, anchor=E)
            else:
                self.tree_right.heading(col, text=col.title(), anchor=W,
                                       command=lambda col_=col: self.sort_text_column(self.tree_right, col_))
                self.tree_right.column(col, minwidth=25, width=75, stretch=NO)
        """Reconfigure this later"""
